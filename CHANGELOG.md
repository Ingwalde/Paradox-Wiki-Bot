...
- **Migrated the whole data layer from SQLite to PostgreSQL.** The stack is now
  ...
  as a second compose service on an internal-only network (the port is never
  published; the password comes from `.env`).
  - All games now share one database, told apart by a `game_key` column, instead
    of one SQLite file per game. The ranking query stays raw SQL, rewritten for
    Postgres: `split_part()` for the `#fragment` strip, `DISTINCT ON (title)` in
    place of the bare-column `GROUP BY` SQLite allowed, `random()` for the random
    page.
  - The writable tables (`uploads`, `feedback`, `search_log`, plus a
    deliberately-unused `api_cache` staged for the next version) are owned by
    Alembic; the bot applies `alembic upgrade head` at startup after waiting for
    the database to come up. Vote deduplication keeps its `ON CONFLICT`
    semantics.
  - The database is now natively async, so the `asyncio.to_thread` wrappers
    around every DB call are gone (the 25 MB gzip in `-tools` stays in a thread —
    it is CPU work, not the database). A single `paradox_bot.storage.StorageError`
    replaces the `sqlite3.Error` handling scattered across eight modules; driver
    exceptions are caught at the storage boundary and re-raised as it.
  - Game data ships as `databases/seed.sql.gz` (generated by
    `scripts/dump_seed.py`, loaded by the official image's
    `docker-entrypoint-initdb.d` on a fresh volume); `scripts/import_wiki.py`
    now writes Postgres. The per-game `databases/*.db` files are removed from git.
  - `/health` returns 503 when a cheap `SELECT 1` fails, so the container is
    marked unhealthy when the database is unreachable rather than reporting a
    half-working bot as fine; the compose `start_period` was raised to match.
  - `DB_DIR` and `DATA_DIR` are gone (no SQLite files to point at); the bot's
    only host-mounted volume is now `logs/`. Backups move from copying files to
