...
async def test_found_results_attach_a_paginated_view(game_db, ctx: FakeContext) -> None:
    for i in range(12):
        await insert_page(game_db, f"Trade node {i}", f"https://wiki/Trade_node_{i}")
...
async def test_the_search_is_remembered_for_the_reaction_handler(
    ...
    await insert_page(game_db, "Absolutism", "https://wiki/Absolutism", "https://img/a.png")
...
async def test_an_over_long_query_is_rejected_before_the_database(
    ...
    monkeypatch.setattr(search_flow.search, "search_pages", explode)
...
async def test_a_database_failure_is_reported_not_raised(
    ...
    monkeypatch.setattr(search_flow.search, "search_pages", broken)
...
async def test_a_suggestion_failure_still_sends_the_not_found_embed(
    ...
    monkeypatch.setattr(search_flow.search, "suggest_similar", broken)
